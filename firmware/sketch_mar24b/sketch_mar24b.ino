...
void runDispenseCycle() {
  ...
  // 3. Monitor Loop
  while (!pillTaken) {
    bool pillDetected = checkUltrasonicForTime(TIME_READ_SENSOR);
    ...
    if (!pillDetected) {
      ...
      if (snoozed) {
        Serial.println("Snoozed via server...");
...
bool checkUltrasonicForTime(unsigned long duration) {
  ...
  while (millis() - start < duration) {
    ...
    if (dist > 0 && dist < 10) detections++;  // adjust threshold
